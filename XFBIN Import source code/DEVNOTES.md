...
tools/
  ...
  PRUEFE_INCLUDES.py      prueft std- und SDK-Header, feste Formatpuffer
  PRUEFE_MELDUNGEN.py     uebersetzt jeden Meldungsaufruf einzeln
...
**Negative Offsets sind Handkorrekturen und die letzte Wahl.**
`across:N` teilt den Rollout in N gleich breite Spalten — bei
Breite 416 und `across:4` sind das rund 104 Pixel je Spalte. Ein
`offset:[-52,…]` schiebt das Element dann mitten in seinen linken
Nachbarn. Erst das Raster passend wählen, dann höchstens um wenige
Pixel nachjustieren.
