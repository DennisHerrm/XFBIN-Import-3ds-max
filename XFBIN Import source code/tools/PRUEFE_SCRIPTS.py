...
# ------------------------------------------------------------
#  Die MaxScript-API - AUS DEM QUELLTEXT gelesen
#
#  Frueher stand hier eine von Hand gepflegte Liste. Die hat
#  genau einmal gelogen, und das reichte: "animHasBones" stand
#  in der Liste, aber nirgends im C++-Teil - eine Zusicherung
#  hatte den Schreibvorgang abgebrochen, ohne dass es auffiel.
#  Die Pruefung sagte "alles sauber", und der Fehler kam erst in
#  3ds Max heraus, hundertvier Mal hintereinander.
#
#  Eine Liste, die man mitpflegen muss, ist keine Pruefung -
#  sie ist eine zweite Stelle, die falsch sein kann. Deshalb
#  wird die API jetzt aus dem Interface-Deskriptor in
#  xfbinimport.cpp gelesen. Dort steht, was das Plugin
#  tatsaechlich veroeffentlicht.
# ------------------------------------------------------------
def read_plugin_api():
    try:
        src = open('src/xfbinimport.cpp', encoding='utf-8').read()
    except OSError:
        return None

    names = set(re.findall(r'_T\("(\w+)"\),\s*0,\s*TYPE_', src))
    return names if names else None


PLUGIN_API = read_plugin_api()
...
def check(path):
    ...
    #
    # Ist der Deskriptor nicht lesbar, wird nicht geprueft statt
    # falsch geprueft - eine Warnung auf duenner Grundlage waere
    # schlechter als keine.
    if PLUGIN_API:
        unknown = sorted(set(re.findall(r'XfbinCpp\.(\w+)', raw)) - PLUGIN_API)
        if unknown:
            problems.append('Unbekannte Plugin-Funktion(en): %s - '
                            'nicht im Deskriptor von xfbinimport.cpp'
                            % ', '.join(unknown))
