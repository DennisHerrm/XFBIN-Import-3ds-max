...
static XfbinImportInterface theXfbinImportInterface(
    ...
    fn_animFrames, _T("animFrames"), 0, TYPE_FLOAT, 0, 1,
        _T("index"), 0, TYPE_INT,
    fn_animHasBones, _T("animHasBones"), 0, TYPE_INT, 0, 1,
        _T("index"), 0, TYPE_INT,
    ...
    fn_pruneScene, _T("pruneScene"), 0, TYPE_INT, 0, 0,
    fn_pendingFolder, _T("pendingFolder"), 0, TYPE_STRING, 0, 0,
...
// ------------------------------------------------------------
//  Meldungen zusammensetzen - ohne feste Puffergroesse
//
//  Die Meldungen dieses Plugins enthalten Namen aus der Datei:
//  Clumps, Bones, Modelle, Pfade. Deren Laenge steht nicht im
//  Voraus fest.
//
//  Mit swprintf_s in einen wchar_t[N] geschrieben ist das eine
//  Falle mit Ansage. In 2kbxspl1.xfbin - einer Spezialangriffs-
//  Animation - werden 39 verschiedene fremde Clumps angesprochen;
//  die Warnung dazu ergibt 656 Zeichen und lief in einen Puffer
//  von 420. swprintf_s erkennt das, ruft den
//  Invalid-Parameter-Handler, und der beendet den Prozess.
//  3ds Max stuerzte mit STATUS_INVALID_PARAMETER ab, ohne dass
//  irgendwo ein Nullzeiger im Spiel war.
//
//  Deshalb bauen alle Meldungen jetzt ueber einen Stringstream.
//  Der waechst mit, und die Frage nach der richtigen
//  Puffergroesse stellt sich nicht mehr.
// ------------------------------------------------------------
namespace {
...
public:
    template <typename T>
    Msg& operator<<(const T& v) { s_ << v; return *this; }

    // Zahlen mit fester Nachkommastelle, wie %.1f
    Msg& Fixed(double v, int digits = 1) {
        const std::streamsize old = s_.precision(digits);
        const std::ios::fmtflags fl = s_.flags();
        s_ << std::fixed << v;
        s_.flags(fl);
        s_.precision(old);
        return *this;
    ...
    std::wstring str() const { return s_.str(); }

    // Damit Log(Msg() << ...) auch ohne abschliessendes .str()
    // uebersetzt. Zwei Aufrufstellen hatten es vergessen, und der
    // Fehler kam erst beim Bauen gegen ein Max-SDK heraus - die
    // Umwandlung nimmt die Moeglichkeit weg, statt sie
    // nachtraeglich zu suchen.
...
int XfbinImportInterface::Dump(const MCHAR* outPath, int includeTables) {
    ...
    Log((Msg() << L"dump geschrieben in ").Fixed(msDump_, 0)
        << L" ms: " << wout);
...
int XfbinImportInterface::ParseSkeleton() {
    ...
    Log(((Msg() << L"parseSkeleton: " << clumps_.size() << L" Clump(s), "
                << bones << L" Bones in ").Fixed(msBones_)
         << L" ms" << (ok ? L"" : L" (mit Hinweisen)")).str());
...
int XfbinImportInterface::BuildSkeletonN(int mode, float scale, int copies) {
    ...
    for (size_t ci = 0; ci < clumps_.size(); ++ci) {
      ...
      for (int copy = 0; copy < nCopies; ++copy) {
        ...
        if (dupes > 0) {
            AddWarning((Msg() << L"Clump '" << Cp932ToWide(clump.name)
                              << L"' hat " << dupes << L" doppelte Bone-Namen. "
                                 L"Fuer den Skin-Modifier muss das eindeutig "
                                 L"sein.").str());
        }
    ...
    if (failed > 0) {
        AddWarning((Msg() << failed
                          << L" Knoten liessen sich nicht anlegen (erster: '"
                          << firstFailure << L"').").str());
    }
...
int XfbinImportInterface::ParseMeshes() {
    ...
    Log(((Msg() << L"parseMeshes: " << models_.size() << L" Modelle, "
                << verts << L" Vertices, " << tris << L" Dreiecke in ")
         .Fixed(msMeshes_)
         << L" ms" << (ok ? L"" : L" (mit Hinweisen)")).str());
...
int XfbinImportInterface::BuildMeshesN(int skipLod, int explicitNormals,
    ...
    Log(((((Msg() << L"buildMeshes: " << made << L" Objekte in ")
           .Fixed(msGeom_)
           << L" ms (" << skipped << L" LOD ausgelassen, "
           << skinned << L" geskinnt in ").Fixed(msSkin_)
           << L" ms, Normalen " << (explicitNormals ? L"explizit" : L"aus")
           << L", Skalierung ").Fixed(static_cast<double>(scale), 3)
         << L")").str());
...
int XfbinImportInterface::ParseAnimsAppend() {
    ...
    Log(((Msg() << L"parseAnims: +" << (anims_.size() - before)
                << L" Animationen (jetzt " << anims_.size() << L"), "
                << keys << L" Keyframes in ").Fixed(msAnims_)
         << L" ms" << (ok ? L"" : L" (mit Hinweisen)")).str());
...
int XfbinImportInterface::BuildAnimAt(int index, float startFrame,
    ...
    for (const xfbin::AnmEntry& entry : anm.entries) {
        ...
        if (!ResolveEntryTarget(anm, entry, clumpSlot, boneSlot)) {
            if (clumpSlot == SIZE_MAX) {
                ++foreignClump;
                // Die Liste begrenzen. 2kbxspl1_atk nennt rund
                // vierzig fremde Clumps; die Warnung waere sonst
                // ueber 600 Zeichen lang. Sie stuerzt seit dem
                // Stringstream nicht mehr ab - aber eine Meldung,
                // die niemand liest, ist auch keine Meldung.
                if (foreignNames.find(entry.clumpName) == std::string::npos) {
                    if (foreignNamesShown < 12) {
                        if (!foreignNames.empty()) foreignNames += ", ";
                        foreignNames += entry.clumpName;
                        ++foreignNamesShown;
                    } else if (foreignNamesShown == 12) {
                        foreignNames += ", ...";
                        ++foreignNamesShown;
                    }
                }
    ...
    if (foreignClump > 0) {
        ...
        AddWarning((Msg() << foreignClump
                          << L" Eintraege gehoeren zu Clumps, die nicht in der "
                             L"Szene stehen ("
    ...
    if (unmatched > 0) {
        AddWarning((Msg() << unmatched
                          << L" Eintraege ohne passenden Bone (erster: '"
                          << firstUnmatched
                          << L"'). Gehoert die Animation zu diesem "
                             L"Modell?").str());
    }

    Log((Msg() << L"buildAnim '" << Cp932ToWide(anm.name) << L"': "
               << keysSet << L" Keys in ").Fixed(msKeys_)
        << L" ms, " << static_cast<int>(anm.frameCount) << L" Frames ab "
        << static_cast<int>(startFrame) << L" (Kanaele: "
        << ((channelMask & 1) ? L"Pos " : L"")
        << ((channelMask & 2) ? L"Rot " : L"")
        << ((channelMask & 4) ? L"Scl"  : L"") << L")");
...
int XfbinImportInterface::BuildVisibility(int index, float startFrame,
    ...
    Log((Msg() << L"buildVisibility '" << Cp932ToWide(anm.name) << L"': "
               << touched << L" Objekte, davon " << hidden
               << L" ausgeblendet").str());
...
int XfbinImportInterface::BuildMaterialAnim(int index, float startFrame,
    ...
    for (const xfbin::AnmEntry& e : anm.entries) {
        ...
        // Ueber das Handle - liefert NULL, wenn das Material
        // seit dem Import geloescht wurde, statt ins Leere zu
        // zeigen.
        Mtl* mtl = static_cast<Mtl*>(Animatable::GetAnimByHandle(it->second));
...
// ------------------------------------------------------------
// ------------------------------------------------------------
//  Hat diese Animation ueberhaupt Bone-Eintraege?
//
//  Ein reiner Kamera- oder Effektclip hat einer Skelettsequenz
//  nichts hinzuzufuegen. Die vier Baufunktionen wuerden ihn
//  folgenlos durchlaufen - aber viermal ueber mehrere tausend
//  Eintraege zu gehen, um nichts zu tun, ist verschenkte Zeit.
//
//  Ausdruecklich KEINE Sicherheitsmassnahme: die Baufunktionen
//  kommen mit solchen Clips zurecht, sie sortieren
//  Nicht-Bone-Eintraege selbst aus.
// ------------------------------------------------------------
int XfbinImportInterface::AnimHasBones(int index) {
    if (!RequireAnims(L"animHasBones")) return 0;
    if (index < 0 || index >= static_cast<int>(anims_.size())) return 0;

    for (const xfbin::AnmEntry& e : anims_[static_cast<size_t>(index)].entries) {
        if (e.entryFormat == xfbin::kEntryBone) return 1;
    }
    return 0;
}
...
// Eintraege wegwerfen, deren Knoten es nicht mehr gibt.
//
// Node-Handles sind schon sicher - GetINodeByHandle liefert NULL.
// Aber eine Liste voller toter Eintraege kostet bei jedem
// Durchlauf Zeit und macht die Zaehlungen falsch.
// ------------------------------------------------------------
//  Uebergabe aus Max' Import-Dialog
//
//  Der Dialog laesst genau eine Datei waehlen - einen Ordner
//  kann er nicht. Das macht aber nichts: aus dem Pfad der
//  gewaehlten Datei ergibt sich ihr Ordner, und den kennt die
//  Oberflaeche als das, womit sie ohnehin arbeitet.
//
//  Wer also irgendeine .xfbin des Charakters waehlt, bekommt
//  denselben Ablauf wie ueber das Menue - mit allen Modell- und
//  Animationsdateien daneben.
// ------------------------------------------------------------
void XfbinImportInterface::SetPendingFolder(const std::wstring& dir) {
    pendingFolder_ = dir;
}

// Die Oberflaeche oeffnen. Ueber das Makro, damit es genau
// dasselbe Fenster ist wie ueber das Menue - eine zweite,
// leicht abweichende Fassung waere die naechste Fehlerquelle.
void XfbinImportInterface::RunScriptText(const std::wstring& script) {
    RunScript(script);
}

void XfbinImportInterface::RunMacro() {
    RunScript(L"try (macros.run \"DH Tools\" \"XfbinImport_Open\") "
              L"catch (format \"[XFBIN] Makro nicht gefunden.\\n\")");
}

const MCHAR* XfbinImportInterface::TakePendingFolder() {
    scratch_ = pendingFolder_;
    pendingFolder_.clear();
    return scratch_.c_str();
}

int XfbinImportInterface::PruneScene() {
    Interface* ip = GetCOREInterface();
    if (ip == nullptr) return 0;

    int dropped = 0;

    for (std::vector<ULONG>& hs : boneHandles_) {
        for (ULONG& h : hs) {
            if (h != 0 && ip->GetINodeByHandle(h) == nullptr) {
                h = 0;
                ++dropped;
            }
        }
    ...
    std::vector<SceneMesh> alive;
    alive.reserve(sceneMeshes_.size());
    for (const SceneMesh& m : sceneMeshes_) {
        if (ip->GetINodeByHandle(m.handle) != nullptr) {
            alive.push_back(m);
        } else {
            ++dropped;
        }
    }
    sceneMeshes_.swap(alive);

    for (auto it = sceneMaterials_.begin(); it != sceneMaterials_.end(); ) {
        if (Animatable::GetAnimByHandle(it->second) == nullptr) {
            it = sceneMaterials_.erase(it);
            ++dropped;
        } else {
            ++it;
        }
...
const MCHAR* XfbinImportInterface::GetSceneReport() {
    ...
    for (size_t i = 0; i < show; ++i) {
        ...
        // Objekt- und Elternnamen sind beliebig lang - deshalb
        // auch hier kein fester Puffer.
        o << L"  " << std::setw(7) << static_cast<int>(r.size) << L"  "
          << std::left << std::setw(28) << r.name << std::right
          << L"  bbox=" << static_cast<int>(r.bb.x)
          << L"/" << static_cast<int>(r.bb.y)
          << L"/" << static_cast<int>(r.bb.z)
          << std::fixed << std::setprecision(2)
          << L"  scale=" << static_cast<double>(r.scale.x)
          << L"/" << static_cast<double>(r.scale.y)
          << L"/" << static_cast<double>(r.scale.z)
          << std::defaultfloat
          << L"  skin=" << skinText
          << L"  parent=" << r.parent << L"\n";
    }
...
int XfbinImportInterface::ExportTextures(const MCHAR* directory) {
    ...
    wchar_t buf[320];
    Log((Msg() << L"exportTextures: " << n << L" DDS nach " << wdir).str());
...
int XfbinImportInterface::BuildMaterials(const MCHAR* directory) {
    ...
    // Innerhalb eines Laufs mit Zeigern arbeiten, gespeichert
    // wird das Handle. Vor dem Lauf einmal aussortieren, was seit
    // dem letzten Import geloescht wurde.
    for (auto it = sceneMaterials_.begin(); it != sceneMaterials_.end(); ) {
        if (Animatable::GetAnimByHandle(it->second) == nullptr) {
            it = sceneMaterials_.erase(it);
        } else {
    ...
    for (const MeshRef& ref : meshNodes_) {
        ...
        for (const xfbin::NudMeshGroup& g : model.groups) {
            if (g.meshes.size() > matNames.size()) {
                wchar_t wbuf[320];
                AddWarning((Msg() << L"Modell '" << Cp932ToWide(model.name)
                                  << L"': " << g.meshes.size()
                                  << L" Submeshes, aber nur " << matNames.size()
                                  << L" Materialien - die ueberzaehligen "
                                     L"bleiben ohne.").str());
                break;
            }
        }
        ...
        for (const xfbin::RawString& mn : matNames) {
            ...
            // Aus einem frueheren Import noch vorhanden?
            auto prev = sceneMaterials_.find(mn);
            if (prev != sceneMaterials_.end()) {
                Mtl* old = static_cast<Mtl*>(
                    Animatable::GetAnimByHandle(prev->second));
                if (old != nullptr) {
                    byName[mn] = old;
                    subs.push_back(old);
                    continue;
                }
                sceneMaterials_.erase(prev);
            }
            ...
            byName[mn] = built;
            if (built != nullptr) {
                sceneMaterials_[mn] = Animatable::GetHandleByAnim(built);
            }
    ...
    if (missing > 0) {
        wchar_t wbuf[320];
        AddWarning((Msg() << missing
                          << L" Materialien verweisen auf Texturen, die nicht "
                             L"in dieser Datei liegen (erste: '"
                          << firstMissing
                          << L"') - sie kommen aus einem gemeinsamen "
                             L"XFBIN.").str());
    }
...
XfbinImportInterface* GetXfbinInterface() {
    return &theXfbinImportInterface;
}
